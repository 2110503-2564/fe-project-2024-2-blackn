...
export default function Booking() {
  ...
  useLayoutEffect(() => {
    const getDentist = async () => {
      try {
        ...
        // console.log("dentistParams", selectedDentist);
      } catch (err) {
        router.push("/");
      } finally {
        setLoading(false);
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-b from-green-100 to-white">
      <div
        className={`flex flex-col items-center px-10 py-20 bg-white rounded-3xl h-[80vh] w-[40vw] max-md:w-[90%] max-sm:px-5 max-[md]:py-14 max-sm:py-10 max-sm:w-[95%] shadow-xl transform /*hover:scale-[1.01]*/ transition-all duration-300`}
      >
